...
class Engine():
    ...
    def initialize_optimizer_and_scheduler(self):
        ...
        self.optimizer = optim.Adam(self.model.get_config_optim(lr),
                                    lr=lr)
        #
    ...
    def initialization(self, is_train):
        if is_train:
            ...
            print('Number of CNN\'s learnable parameters: ' +
                  str(self.count_learnabel_parameters(self.learnabel_parameters(self.model.cnn))))
            print('Number of GCN_1\'s learnable parameters: ' +
                  str(self.count_learnabel_parameters(self.learnabel_parameters(self.model.gc1))))
            print('Number of GCN_2\'s learnable parameters: ' +
                  str(self.count_learnabel_parameters(self.learnabel_parameters(self.model.gc2))))
    ...
    def load_model(self):
        self.model.cnn.load_state_dict(torch.load(self.model.cnn.path))
        self.model.gc1.load_state_dict(torch.load(self.model.gcn1_path))
        self.model.gc2.load_state_dict(torch.load(self.model.gcn2_path))

    def save_model(self):
        torch.save(self.model.cnn.state_dict(), self.model.cnn.path)
        torch.save(self.model.gc1.state_dict(), self.model.gcn1_path)
        torch.save(self.model.gc2.state_dict(), self.model.gcn2_path)
    ...
    def train(self, dataloader, epoch=None, thresholds=0.5):
        ...
        for batch_idx, (images, targets) in enumerate(tqdm(dataloader)):
            ...
            # the model
            outputs = self.model(images, self.emb, self.adj)
        ...
        if not mcc:
            if results['per_class/f1'] > self.best_f1_score:
                ...
                lines = ['Loss Function: ' + str(self.args.loss_function),
                         ...
                         per_class_metrics,
                         per_image_metrics]
                with open(self.args.save_dir + 'CNN_GCN_validation_results.txt', 'w') as f:
                    f.write('\n'.join(lines))
                f.close()
                #
                self.best_f1_score = results['per_class/f1']
